---
name: kanzlei-lebenszyklus-output-versand
...
Wenn der Text aus dem Schreib-Canvas kommt und juristisch noch unsubstantiiert wirkt, vor Versand `kanzlei-lebenszyklus-freundlicher-copilot`, `kanzlei-lebenszyklus-schreibcanvas` und `kanzlei-lebenszyklus-qualitaetsgate-hardening` einschalten. Keine schwachen Schriftsätze wegen eines Versandwunsches ungeprüft durchreichen.

Bei Klage, Replik oder gerichtlichem Antrag vor Versand zusätzlich `kanzlei-lebenszyklus-schriftsatz-turbo` und das Anlagenverzeichnis prüfen.
...
Nach jedem technisch bestätigten beA-Versand immer `kanzlei-lebenszyklus-bea-journal` ausführen oder anbieten:
...
6. Fristen- oder EB-Folgen an `kanzlei-lebenszyklus-fristen-monitor` übergeben.
